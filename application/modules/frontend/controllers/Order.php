	...
	$this->form_validation->set_rules('nama', 'Nama', 'trim|required');
...

		{
...

			{
			 ...
			 //ambil jumlah perproduk di db
			 $connect = mysqli_connect("localhost","root","","db_toko1");
			 $query1 = mysqli_query($connect, "SELECT jumlah from produk where nama_produk like '%$nama_produk%'");
			 $data1 = mysqli_fetch_array($query1);
			 $data_jumlah = $data1['jumlah'];
			 $tes = $data_jumlah-$item['qty']; //kurangi jumlah di db sama hasil order_detail
			 $dataupd = array(
			        'table_name' => 'produk', // nama table sql
			        'nama_produk' => $item['name'],
			        'jumlah' => $tes,
			    );
			 $this->Frontend_model->upddata($dataupd);
		    }
